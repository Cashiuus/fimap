...
class XML2Config(baseClass):
    ...
    def __init_xmlresult(self):
        ...
        if (os.path.exists(xmlfile)):
            ...
            self._log("Processing generic.xml :: relative_files loaded into list of size: {}".format(repr(len(self.relative_files))), self.LOG_DEBUG)
            ...
            self._log("Processing generic.xml :: absolute_files loaded into list of size: {}".format(repr(len(self.absolute_files))), self.LOG_DEBUG)
            ...
            self._log("Processing generic.xml :: remote_files loaded into list of size: {}".format(repr(len(self.remote_files))), self.LOG_DEBUG)
            ...
            self._log("Processing generic.xml :: log_files loaded into list of size: {}".format(repr(len(self.log_files))), self.LOG_DEBUG)
            ...
            self._log("Processing generic.xml :: Mindepth (%d) and Maxdepth (%d) loaded" % (mindepth, maxdepth), self.LOG_DEBUG)

            blind_files = getXMLNodes(blind_node, "file")
            for f in blind_files:
                self.blind_files.append(fiFile(f, self.config))
            self._log("Processing generic.xml :: blind_files loaded into list of size: {}".format(repr(len(blind_files))), self.LOG_DEBUG)
            ...
            print("[ERR] generic.xml file not found! This file is very important!")
            self._log("Processing generic.xml :: Failed to locate, file is missing!", self.LOG_DEBUG)
            sys.exit(1)
    ...
    def __loadLanguageSets(self):
        ...
        for c in langnodes.childNodes:
            if (c.nodeName == "language"):
                ...
                self._log("Process language XMLs :: Loaded XML-LD for '%s' at revision %d by %s" %(langname, langClass.getRevision(), langClass.getAutor()), self.LOG_DEBUG)
...
class baseLanguage(baseTools):
    ...
    def __init__(self, langname, langfile, config):
        ...
        if (os.path.exists(langfile)):
            ...
            self._log("Processing extra language set XML files :: {} file not found!".format(langfile), self.LOG_DEBUG)
            #sys.exit(1)
    ...
    def generateQuiz(self):
        ...
        try:
            ...
            boxarr.append("If not please report this bug (!) Thanks!")
    ...
    def __populate(self):
        ...
        if (len(self.exec_methods) == 0):
            self._log("{} xml :: This XML-LD has no exec-method(s) defined!".format(self.LanguageName), self.LOG_ERROR)
            self._log("{} xml :: This XML-LD can't be used to go into exploit mode!".format(self.LanguageName), self.LOG_ERROR)
        ...
        if (len(self.payloads) == 0):
            self._log("{} xml :: This XML-LD has no payload(s) defined!".format(self.LanguageName), self.LOG_DEBUG)
        ...
        if (self.sniper_regex == None or self.sniper_regex.strip() == ""):
            self._log("{} xml :: This XML-LD has no sniper regex! So this XML-LD can only be used in blind-mode!".format(self.LanguageName), self.LOG_WARN)

        methods_node = getXMLNode(self.XML_Rootitem, "methods")
        quiz_node = getXMLNode(methods_node, "quiz")
        if (quiz_node == None):
            self._log("{} xml :: FATAL! XML-Language-Definition has no quiz function defined!".format(self.LanguageName), self.LOG_ERROR)
        ...
        if (print_node == None):
            self._log("%s XML-Language-Definition :: has no print function defined!"%(self.getName()), self.LOG_ERROR)
            self._log("Please fix that in order to run fimap without problems!", self.LOG_ERROR)
            self._log("Committing suicide :-O", self.LOG_ERROR)
            sys.exit(1)
        else:
        ...
        if (eval_node == None):
            self._log("%s XML-LD :: has no eval_kickstarter method defined."%(self.getName()), self.LOG_DEBUG)
        ...
        if (write_node == None):
            self._log("%s XML-LD :: has no write_file method defined."%(self.getName()), self.LOG_DEBUG)
